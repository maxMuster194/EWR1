...
export default function Details({ data = [], uniqueDates = [], todayBerlin, error }) {
  ...
  const chartData = {
    labels: priceFields.map((field, index) => {
      console.log('Field:', field, 'Index:', index); // Debug
      const [hourPart, quarter] = field.split(' ');
      console.log('HourPart:', hourPart, 'Quarter:', quarter); // Debug
      let hourNum;
      if (hourPart === 'Hour 3A') {
        hourNum = 3;
      } else {
      ...
      console.log('HourNum:', hourNum, 'QuarterNum:', quarterNum); // Debug
  ...
  return (
    <div className="min-h-screen p-4" style={{ backgroundColor: '#fafafa' }}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <label htmlFor="date-picker" className="block text-sm font-medium text-red-700">
            Datum auswählen:
          </label>
          <DatePicker
            id="date-picker"
