...
export default function DynamischerPreis({ data = [], uniqueDates = [], todayBerlin, error }) {
  ...
  const chartData = {
    labels: priceFields.map((field, index) => {
      // Korrigierte Labels-Generierung: Direkt aus index berechnen, keine fehlerhaften Splits
      let hourNum;
    ...
    datasets: filteredData.length > 0 ? [{
      ...
      borderColor: '#063d37',
      backgroundColor: 'rgba(6, 61, 55, 0.2)', // Transparente Farbe für Fill (Schattierung unten)
      fill: true, // Aktiviert Fill unter der Linie für Hintergrundschattierung (inkl. 23:45)
      tension: 0.4,
      spanGaps: true, // Verhindert Lücken in der Linie bei fehlenden Daten
    }] : [],
  };
