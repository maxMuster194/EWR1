...
export default function Home({ data = [], uniqueDates = [], todayBerlin, error: propError }) {
  ...
  const chartDataKosten = {
    ...
    datasets: [
      // Entfernt: Dynamischer Preis-Dataset, da er nicht direkt angezeigt werden soll; stattdessen nur berechnete Kosten für Geräte
      {
        label: 'Kosten Dynamischer Tarif (Ct/h)',
        data: hourlyData.flatMap((d, h) => Array(4).fill().map((_, q) => d.total * chartConvertedValues[h * 4 + q])),
        fill: false,
        borderColor: '#000000',
  ...
  const chartOptionsKosten = {
      ...
      tooltip: {
        callbacks: {
          label: function(context) {
            const label = context.dataset.label || '';
            const value = context.raw.toFixed(2);
            const unit = label.includes('Preis') ? 'Ct/kWh' : 'Ct/h';
    ...
    scales: {
      y: {
        beginAtZero: true,
        title: { display: true, text: 'Preis (Ct/kWh) / Kosten (Ct/h)', color: '#333' },
        ticks: { color: '#333' },
      },
      x: {
        title: { display: true, text: 'Uhrzeit', color: '#333' },
