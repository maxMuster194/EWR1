...
const berechneDynamischeKosten = (verbraucher, strompreis, pricesApi, verbraucherDaten, erweiterteEinstellungen, selectedRegion) => {
  ...
  if (type === 'grundlast') {
    ...
    let totalWeight = 0;
    const slotDuration = 0.25;
    for (let q = 0; q < pricesApi.length; q++) {
      const frac = slotDuration;
      totalWeightedPrice += (pricesApi[q] || fallbackPrice) * frac;
      totalWeight += frac;
    }
  ...
  einstellung.zeitraeume.forEach((z) => {
    ...
    while (remaining > 0) {
      ...
      const slotStart = currentSlot * slotDuration;
      const slotEnd = slotStart + slotDuration;
      const frac = Math.min(slotEnd - currentTime, remaining);
      const price = pricesApi[currentSlot % pricesApi.length] || fallbackPrice;
      totalWeightedPrice += price * frac;
      totalWeight += frac;
      remaining -= frac;
      currentTime += frac;
...
const berechneStundenVerbrauch = (verbraucherDaten, erweiterteEinstellungen) => {
  const numSlots = 96;
  const slotDuration = 0.25;
  const stunden = Array(numSlots).fill(0).map(() => ({
    grundlast: 0,
    dynamisch: 0,
    waermepumpe: 0,
    total: 0,
    ...
    if (isGrundlast) {
      ...
      einstellung.zeitraeume?.forEach(zeitraum => {
        ...
        while (remaining > 0) {
          ...
          const slotEnd = slotStart + slotDuration;
          const frac = Math.min(slotEnd - currentTime, remaining);
          const add = (useWatt / 1000) * (frac / slotDuration) * dailyMultiplier;
          stunden[currentSlot % numSlots][category] += add;
          stunden[currentSlot % numSlots].total += add;
          if (!stunden[currentSlot % numSlots].verbraucher.includes(verbraucher)) {
            stunden[currentSlot % numSlots].verbraucher.push(verbraucher);
          }
          remaining -= frac;
          currentTime += frac;
          if (currentTime >= 24) currentTime -= 24;
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch('/api/min15prices', {
          cache: 'no-store',
        });
        if (!res.ok) {
        ...
        setApiData(min15Data);
        const uniqueDates = [...new Set(
          min15Data
            .map((entry) => entry['Delivery day'])
  ...
  const { chartDataApi, chartConvertedValues, chartLabels } = useMemo(() => {
    const labels = [];
    const prices = [];
    const entry = apiData.find(e => e['Delivery day'] === selectedDate);
    if (!entry) {
      const defaultLabels = [];
      for (let h = 0; h < 24; h++) {
        for (let m = 0; m < 4; m++) {
          defaultLabels.push(`${h.toString().padStart(2, '0')}:${(m * 15).toString().padStart(2, '0')}`);
        }
      }
      return { chartDataApi: [], chartConvertedValues: [], chartLabels: defaultLabels };
    }

    for (let h = 1; h <= 24; h++) {
      if (h === 3) {
        let added = false;
        if (entry['Hour 3A Q1'] != null) {
          for (let q = 1; q <= 4; q++) {
            prices.push(entry[`Hour 3A Q${q}`]);
          }
          for (let m = 0; m < 4; m++) {
            labels.push(`02:${(m * 15).toString().padStart(2, '0')}`);
          }
          added = true;
        }
        if (entry['Hour 3B Q1'] != null) {
          for (let q = 1; q <= 4; q++) {
            prices.push(entry[`Hour 3B Q${q}`]);
          }
          for (let m = 0; m < 4; m++) {
            labels.push(`03:${(m * 15).toString().padStart(2, '0')}`);
          }
          added = true;
        }
        if (!added) {
        ...
        for (let q = 1; q <= 4; q++) {
          prices.push(entry[`Hour ${h} Q${q}`]);
        }
        const hour = h - 1;
        for (let m = 0; m < 4; m++) {
          labels.push(`${hour.toString().padStart(2, '0')}:${(m * 15).toString().padStart(2, '0')}`);
        }
      }
    }

    const chartConvertedValues = prices.map(v => (v / 10) || strompreis);
    const chartDataApi = labels.map((label, i) => ({ label, value: prices[i], index: i }));
    return { chartDataApi, chartConvertedValues, chartLabels: labels };
      ...
      if (verbraucherDaten[key]?.watt > 0 || verbraucherDaten[key]?.checked) {
        ...
        if (verbraucherTypes[key] === 'grundlast') {
          ...
          if (jaz !== 0) {
            ...
            if (einstellung.zeitraeume && einstellung.zeitraeume.length > 0) {
              einstellung.zeitraeume.forEach((z, index) => {
                ...
                while (remaining > 0) {
                  ...
                  const slotEnd = slotStart + slotDuration;
                  const frac = Math.min(slotEnd - currentTime, remaining);
                  const apiPrice = pricesApi[currentSlot % pricesApi.length] || parseFloat(getPreisDifferenz(strompreis, selectedRegion));
  
                  // Debugging: Ausgabe der Stunde und des abgerufenen Preises
                  console.log(`Slot: ${currentSlot}, API-Preis: ${apiPrice}, Fraktion: ${frac}`);
              ...
              for (let q = 0; q < pricesApi.length; q++) {
                const apiPrice = pricesApi[q] || parseFloat(getPreisDifferenz(strompreis, selectedRegion));
                const frac = slotDuration;
                console.log(`Slot: ${q}, API-Preis: ${apiPrice}`);
                totalWeightedPrice += apiPrice * frac;
                totalWeight += frac;
              }
            }
            ...
            dynselbstbestimmt += (((verbraucherDaten[key].watt/1000) / jaz) * (heizstunden / 24) * weightedPrice / 100);  // Skalierung auf Jahr anpassen
          }
        } else {
...
const chartData = {
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: { display: true, text: '15-minütlicher Stromverbrauch und Preis', color: '#333', font: { size: 11.2 } },
      tooltip: {
        callbacks: {
  ...
  const chartDataKosten = {
    ...
    datasets: [
      {
        label: 'Kosten Dynamischer Tarif (Ct per 15 min)',
        data: hourlyData.map((_, index) => {
          ...
          return (((hourlyData[index].total - hourlyData[index].waermepumpe) * price * 0.25).toFixed(2));
        }),
        fill: false,
        borderColor: '#88bf50',
        backgroundColor: '#88bf50',
      ...
      {
        label: 'Kosten Fester Tarif (Ct per 15 min)',
        data: hourlyData.map((_, index) => {
      ...
      {
        label: 'Kosten Wärmepumpe (Ct per 15 min)',
        data: hourlyData.map((_, index) => {
          const price = chartConvertedValues[index] != null ? chartConvertedValues[index] : parseFloat(getPreisDifferenz(strompreis, selectedRegion));
          return ((hourlyData[index].waermepumpe * price * 0.25).toFixed(2));
        }),
        fill: false,
        borderColor: '#f93b01',
        backgroundColor: '#f93b01',
  ...
  const chartOptionsKosten = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `15-minütliche Stromkosten (inkl. Wärmepumpe) am ${selectedDate || 'Fallback-Preis'}`,
        color: '#333',
        font: { size: 11.2 },
      },
      tooltip: {
        callbacks: {
          label: function(context) {
            ...
            const isDynamicNonWP = datasetLabel.includes('Dynamischer Tarif') && !datasetLabel.includes('Wärmepumpe');
            const isFixedNonWP = datasetLabel.includes('Fester Tarif');
            const isWpDynamic = datasetLabel.includes('Kosten Wärmepumpe');
    ...
    scales: {
      y: {
        beginAtZero: true,
        title: { display: true, text: 'Kosten (Ct per 15 min)', color: '#333' },
        ticks: { color: '#333' },
      },
      x: {
