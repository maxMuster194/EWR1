...
const updateMongoDB = async (data, model) => {
  try {
    ...
    throw err; // Fehler weiterwerfen, um in updateData zu handhaben
  }
};
...
const updateData = async () => {
  ...
  try {
    ...
    console.log('erfolgreich'); // Nur "erfolgreich" in der Konsole bei Erfolg
    return { germanyPrices, austriaPrices }; // Daten zurückgeben für JSON-Ausgabe
  } catch (error) {
    console.log('Fehler'); // Nur "Fehler" in der Konsole bei Fehler
    throw error; // Fehler weiterwerfen für API-Handler
  }
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({ ...data, message: "Daten erfolgreich aktualisiert" }); // JSON mit Daten und Message zurückgeben
  } catch (error) {
...
cron.schedule('10 14 * * *', async () => {
  ...
  try {
    ...
    console.log('Fehler bei geplanter Aktualisierung');
  }
}, {
...
(async () => {
  try {
    ...
    console.log('Fehler bei initialer Ausführung');
  }
})();