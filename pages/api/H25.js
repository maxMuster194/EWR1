...
// Schema für H25-Collection
const H25Schema = new mongoose.Schema({
...
// Modell
const H25Model = mongoose.models.H25Model || mongoose.model('H25Model', H25Schema);
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET') {
      const data = await H25Model.find({}).lean();
      if (!data || data.length === 0) {
        return res.status(404).json({ error: 'No data found in H25 collection' });
      }
      ...
      return res.status(200).json(formattedData); // Entferne die unnötige 'h25'-Ebene
    } else {
