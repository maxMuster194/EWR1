                    ...
                    <TableRow sx={{ bgcolor: 'action.selected' }}>
                      ...
                      <TableCell>
                        {(() => {
                          ...
                          const validAverages = weeklyAverages.filter((week) => week.average !== '–').map((week) => parseFloat(week.average));
                          return validAverages.length > 0 ? (validAverages.reduce((sum, val) => sum + val, 0) / validAverages.length).toFixed(2) : '–';
                        })()}
                      </TableCell>
                      <TableCell>
                        {(() => {
                      ...
                      <TableCell>
                        {(() => {
                          const weeklyAverages = calculateWeeklyAverages(selectedMonth, monthlyData[selectedMonth]);
                          const monatsVerbrauch = parseFloat(monthlyKwh[selectedMonth]) || 0;
                          const totalKosten = weeklyAverages.filter((week) => week.average !== '–').reduce((sum, week) => sum + (parseFloat(week.average) * monatsVerbrauch) / 100 / 4, 0);
                      ...
                      <TableCell>
                        {(() => {
                          const weeklyAverages = calculateWeeklyAverages(selectedMonth, monthlyData[selectedMonth]);
                          const monatsVerbrauch = parseFloat(monthlyKwh[selectedMonth]) || 0;
                          const totalKostenEigenerPreis = weeklyAverages.filter((week) => week.average !== '–').reduce((sum) => sum + (adjustedPrice * monatsVerbrauch) / 100 / 4, 0);
