...
export default function Home() {
  ...
  // Helper function to format time periods for cost breakdown
  const formatZeitraeume = (zeitraeume) => {
    if (!zeitraeume || zeitraeume.length === 0) return '-';
    return zeitraeume
      .map(z => `${z.startzeit}-${z.endzeit} (${z.dauer}h)`)
      .join(', ');
  };
    ...
    <div className="app-container">
      ...
      <div className="calculation-report">
        ...
        <div className="report-content">
          ...
          <div className="input-container-html date-picker-container">
            <label htmlFor="date-picker">Datum für dynamischen Preis:</label>
            <select
              id="date-picker"
          ...
          {menus.map((menu) => (
            <div key={menu.id} className={`menu ${menu.id === 'dynamischeverbraucher' ? 'dynamischeverbraucher' : ''}`}>
              <div
              ...
              <div className={`menu-content ${openMenus[menu.id] ? '' : 'hidden'}`}>
                <ul className={`checkbox-group ${menu.id === 'dynamischeverbraucher' ? 'dynamischeverbraucher' : ''}`}>
          ...
          <div className="summary-container">
            ...
            <h4 className="summary-title">Kostenaufschlüsselung</h4>
            <table className="cost-breakdown-table">
              <thead>
                <tr className="table-header">
                  <th>Verbraucher</th>
                  <th>Wattleistung (W)</th>
              ...
              <tbody>
                {Object.keys(verbraucherDaten)
                  .filter(
                    (verbraucher) =>
                      verbraucherDaten[verbraucher].checked ||
                      verbraucherDaten[verbraucher].watt > 0
                  )
                  .map((verbraucher) => (
                    <tr key={verbraucher} className="table-row">
                      <td>{verbraucher}</td>
                      <td>{verbraucherDaten[verbraucher].watt || 0}</td>
                      <td>
                        {isDynamisch(verbraucher)
                          ? `${erweiterteEinstellungen[verbraucher]?.nutzung || 0}x pro Woche`
                          : 'Kontinuierlich'}
                      </td>
                      <td>
                        {isDynamisch(verbraucher)
                          ? formatZeitraeume(erweiterteEinstellungen[verbraucher]?.zeitraeume)
                          : '24/7'}
                      </td>
                      <td>{verbraucherDaten[verbraucher].kosten || '0.00'}</td>
                    </tr>
                  ))}
              </tbody>
            </table>
      ...
      <div className="diagrams-container">
        <div className="diagram">
          <h3 className="diagram-title">Stündlicher Stromverbrauch und Preis</h3>
          <div className="chart-container">
            <Line data={chartData} options={chartOptions} />
          </div>
        </div>
        <div className="diagram">
          <h3 className="diagram-title">Stündliche Stromersparnis</h3>
          <div className="chart-container">
