...
const getPreisDifferenz = (strompreis, selectedRegion) => {
  ...
  if (regionPreis !== null) {
    return (strompreis - regionPreis).toFixed(2); // Strompreis - Regionspreis
  }
  return strompreis; // Falls keine Region ausgewählt, ursprünglichen Strompreis verwenden
};
...
const updateKosten = (watt, verbraucher, strompreis, selectedRegion, setVerbraucherDaten, erweiterteEinstellungen) => {
  ...
  if (kosten < 0) kosten = 0; // Sicherstellen, dass keine negativen Kosten entstehen
...
export default function Home() {
  const [strompreis, setStrompreis] = useState(0.31); // Standard-Strompreis 0,31 €
  const [selectedRegion, setSelectedRegion] = useState(null); // Für die Region (KF, MN, MOD)
  ...
  const handleRegionChange = (region, offset) => {
    // Wenn die ausgewählte Region erneut angeklickt wird, deaktiviere sie
    const newRegion = selectedRegion === region ? null : region;
    setSelectedRegion(newRegion);
  
    // Aktualisiere die Kosten für alle Verbraucher
    ...
    setVerbraucherDaten((prev) => {
      ...
      if (checked) {
        if (type !== 'grundlast') {
          ...
          kosten = (watt * getPreisDifferenz(strompreis, selectedRegion) * 24 * 365) / 1000;
          if (kosten < 0) kosten = 0; // Sicherstellen, dass keine negativen Kosten entstehen
        }
      }
  ...
  const handleStrompreisChange = (value) => {
    const newStrompreis = parseFloat(value) || 0.31; // Fallback auf 0.31 wenn ungültig
    if (newStrompreis < 0) {
      setError('Strompreis darf nicht negativ sein.');
      return;
    }
    setStrompreis(newStrompreis);
    setSelectedRegion(null); // Region deaktivieren, wenn Strompreis manuell geändert wird
    setError('');
    Object.keys(verbraucherDaten).forEach((verbraucher) => {
      const { watt, checked } = verbraucherDaten[verbraucher];
      if (checked || watt > 0) {
    ...
    datasets: [
      {
        ...
        data: Array(24).fill(0).map(() => {
          ...
          return (verbraucherDaten.totalWattage * price).toFixed(2);
        }),
        fill: false,
        borderColor: '#062316',
        backgroundColor: '#062316',
      ...
      tooltip: {
        callbacks: {
          label: function(context) {
            ...
            const price = isDynamic ? (chartConvertedValues[index] != null ? chartConvertedValues[index] : getPreisDifferenz(strompreis, selectedRegion)) : getPreisDifferenz(strompreis, selectedRegion);
            const verbraucherList = hourlyData[index].verbraucher.join(', ');
  ...
  <div className="region-switch-wrapper">
    ...
    <div className="discount-switch-container">
      <input
        ...
        onChange={() => handleRegionChange('KF', 10)}
  ...
  <div className="region-switch-wrapper">
    ...
    <div className="discount-switch-container">
      <input
        ...
        onChange={() => handleRegionChange('MN', 17)}
  ...
  <div className="region-switch-wrapper">
    ...
    <div className="discount-switch-container">
      <input
        ...
        onChange={() => handleRegionChange('MOD', 20)}
