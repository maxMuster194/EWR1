...
export default function MongoDBPricesPage() {
  ...
  if (loading) {
    return (
      <div className="text-center p-10 text-2xl text-red-600 bg-red-50 rounded-2xl">
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 bg-white rounded-2xl shadow-lg">
      <h1 className="text-3xl font-bold text-gray-800 text-center mb-8">Durchschnittspreise 2025</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center flex-wrap">
          <div className="relative">
            <input
              ...
              className="p-3 text-lg border-2 border-teal-500 rounded-lg w-64 bg-white text-gray-800 focus:border-red-600 focus:shadow-md transition-all"
            ...
            <button
              ...
              className={`px-4 py-2 text-lg font-medium rounded-lg transition-all ${
                selectedDiscount?.label === discount.label
                  ...
                  : 'bg-teal-500 text-white hover:bg-red-600'
              }`}
            ...
            <div
              ...
              className={`p-6 rounded-xl shadow-md cursor-pointer text-center transition-all ${
                selectedMonth === monthKey
                  ? 'bg-gradient-to-br from-red-600 to-red-800 text-white transform -translate-y-1'
                  : 'bg-gradient-to-br from-teal-500 to-teal-700 text-white hover:-translate-y-1 hover:shadow-lg'
              }`}
            >
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-8">
        <h2 className="text-xl font-semibold text-gray-800 text-center mb-4">
          Monatliche Durchschnittspreise 2025
        </h2>
        <Line
          data={{
            labels: months.map((month, index) => monthNames[index]),
            datasets: [
              {
                label: 'Monatlicher Durchschnittspreis (Cent/kWh)',
                data: months.map((monthKey) =>
                  monthlyData[monthKey]
                    ? parseFloat(calculateMonthlyAverage(monthlyData[monthKey]))
                    : null
                ),
                fill: false,
                borderColor: '#D9043D',
                ...
                pointBorderColor: '#b00333',
                pointHoverBackgroundColor: '#b00333',
                pointHoverBorderColor: '#b00333',
                tension: 0.1,
              },
            ],
          }}
          ...
          <div className="bg-white rounded-2xl p-6 max-w-3xl w-full max-h-[85vh] overflow-y-auto relative">
            ...
            {monthlyData[selectedMonth] && monthlyData[selectedMonth].length > 0 ? (
              <>
                ...
                <div className="bg-teal-500 text-white p-4 rounded-lg mb-4 flex items-center gap-3">
                      ...
                      {calculateWeeklyAverages(selectedMonth, monthlyData[selectedMonth]).map((week, i) => {
                        ...
                        return (
                          <tr key={week.kw} className={i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                            <td className="p-3">KW {week.kw}</td>
                            <td className="p-3">{getWeekDateRange(week.days)}</td>
                            <td className="p-3">{week.average}</td>
                            <td className="p-3">{weeklyKwh}</td>
                            <td className="p-3">{kosten}</td>
                            <td className="p-3">{kostenEigenerPreis}</td>
                          </tr>
                        );
                      })}
                    </tbody>
