    ...
    datasets: [
      {
        ...
        data: hourlyData.map((_, index) => {
          const apiPrice = chartConvertedValues[index] != null ? chartConvertedValues[index] : parseFloat(getPreisDifferenz(strompreis, selectedRegion));  // FIX: Fallback auf Differenz
          const ersparnis = hourlyData[index].total * (strompreis - getRegionPreis(selectedRegion) - apiPrice);
          return ersparnis.toFixed(2);
        }),
        fill: false,
        borderColor: '#88bf50',
        backgroundColor: '#88bf50',
      ...
      },
      {
        ...
        data: hourlyData.map((_, index) => {
          const ersparnis = hourlyData[index].total * parseFloat(getPreisDifferenz(strompreis, selectedRegion));
          return ersparnis.toFixed(2);
        }),
        fill: false,
        borderColor: '#063d37',
      ...
      tooltip: {
        callbacks: {
          label: function(context) {
            ...
            const apiPrice = chartConvertedValues[index] ?? parseFloat(getPreisDifferenz(strompreis, selectedRegion));
            const price = isDynamic ? (getRegionPreis(selectedRegion) + apiPrice) : getRegionPreis(selectedRegion);
            const verbraucherList = hourlyData[index].verbraucher.join(', ');
