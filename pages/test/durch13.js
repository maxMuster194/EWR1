...
export default function MongoDBPricesPage() {
  ...
  if (loading) {
    return (
      <div className="text-center p-10 text-2xl text-[#062316] bg-white rounded-2xl">
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 bg-white rounded-2xl shadow-lg">
      ...
      <div className="bg-white p-6 rounded-xl shadow-md mb-8">
        ...
        <div className="flex flex-col sm:flex-row gap-6 justify-center items-start">
          <div className="flex flex-col gap-4">
            <p className="text-sm text-[#333] text-center sm:text-left">
              {selectedDiscount === null
                ? `Keine Region ausgewählt, Preis: ${parseFloat(eigenerPreis).toFixed(2) || 'N/A'} Cent/kWh`
                : `Ausgewählte Region: ${
                    discounts.find((r) => r.label === selectedDiscount)?.label
                  }, Angepasster Preis: ${getAdjustedPrice().toFixed(2)} Cent/kWh`}
            </p>
            <div className="flex flex-col gap-4">
              {discounts.map((discount) => (
                <div key={discount.label} className="flex items-center gap-2">
                  <span className="text-base font-medium text-[#062316]">{discount.label}</span>
                  <label className="relative inline-block w-[50px] h-[24px]">
                    <input
                      ...
                      className="opacity-0 w-0 h-0 discount-switch-input"
                    />
                    <span
                      className={`absolute cursor-pointer top-0 left-0 right-0 bottom-0 rounded-full transition-all duration-400 discount-switch-slider ${
          ...
          <div className="flex flex-col gap-4 w-full sm:w-auto">
            <div className="relative">
              <input
                ...
                className="p-3 text-lg border-2 border-[#062316] rounded-lg w-64 bg-white text-[#062316] focus:border-[#409966] focus:shadow-md transition-all"
      ...
      {isModalOpen && selectedMonth && (
        <div className="fixed inset-0 bg-[#062316] bg-opacity-60 flex justify-center items-center z-50">
          <div className="bg-white rounded-2xl p-6 max-w-3xl w-full max-h-[85vh] overflow-y-auto relative">
            <button
              onClick={closeModal}
              className="absolute top-4 right-4 text-2xl text-[#FF0000] hover:text-[#CC0000] transition-all"
              title="Schließen"
            >
              ×
            </button>
            {monthlyData[selectedMonth] && monthlyData[selectedMonth].length > 0 ? (
              <>
                <h2 className="text-2xl font-semibold text-[#062316] text-center mb-4">
                  {monthNames[parseInt(selectedMonth.split('/')[0]) - 1]} {selectedMonth.split('/')[1]}
                </h2>
                <div className="bg-[#062316] text-white p-4 rounded-lg mb-4 flex items-center gap-3">
                ...
                <div className="bg-[#062316] text-white p-4 rounded-lg mb-4 flex items-center gap-3">
                  ...
                  {selectedDiscount && (
                    <span className="text-sm"> ({selectedDiscount} Rabatt: -{discounts.find((r) => r.label === selectedDiscount)?.value}¢)</span>
                  )}
                </div>
                <div className="bg-white p-4 rounded-lg mb-6">
                  <h3 className="text-xl font-semibold text-[#062316] text-center mb-4">
                  ...
                  <table className="w-full border-collapse bg-white rounded-lg shadow-md">
                      ...
                      {calculateWeeklyAverages(selectedMonth, monthlyData[selectedMonth]).map((week, i) => {
                        ...
                        return (
                          <tr key={week.kw} className={i % 2 === 0 ? 'bg-[#e6e6bf]' : 'bg-white'}>
                            <td className="p-3 text-[#062316]">KW {week.kw}</td>
                            <td className="p-3 text-[#062316]">{getWeekDateRange(week.days)}</td>
                            <td className="p-3 text-[#062316]">{week.average}</td>
                            <td className="p-3 text-[#062316]">{weeklyKwh}</td>
                            <td className="p-3 text-[#062316]">{kosten}</td>
                            <td className="p-3 text-[#062316]">{kostenEigenerPreis}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                    <tfoot>
                      <tr className="bg-[#e6e6bf]">
                        <td className="p-3 font-bold text-[#062316]" colSpan={2}>Gesamt</td>
                        <td className="p-3 font-bold text-[#062316]">
                          {(() => {
                ...
                {calculateWeeklyAverages(selectedMonth, monthlyData[selectedMonth]).filter((week) => week.average !== '–').length > 0 && (
                  <div className="bg-white p-6 rounded-lg shadow-md">
                    <h2 className="text-xl font-semibold text-[#062316] text-center mb-4">
                    ...
                    />
                    <div className="bg-white p-4 rounded-lg mt-6 text-center">
                      <div className="text-lg font-medium text-[#062316] mb-2">
                      ...
                      <div className="text-lg font-medium text-[#062316]">
